...
function drawScene()
{
    ...
    //Passage des variables au shader
    var pos = vec3.create();
...
/*
*   permet de trouver si le rayon tire rencontre une sphere
*   retourne la distance entre le point d'origine et l'intersection
 */
function intersectSphere(dir, origin, spherePosition, sphereRayon)
{
    //equation du deuxième degré pour trouver l'intersection entre droite et sphere

    //trouver le vecteur l spherePosition->Origin
    var l = vec3.fromValues(0.0, 0.0, 0.0);
    vec3.sub(l, origin, spherePosition);

    //trouver les valeurs de l'équation
    ...
    if (discriminant > 0.0)
    {
        ...
        //renvoyer la plus petite (pour la première intersections avec la sphere)
        if( t1 > t2)
        {
            t1 = t2;
        }
...
/*
*   Analyse le "tir" du rayon de la souris en le calculant pour voir si il crois une sphere
*   Renvoie la distance de la sphere croisée ou 1000.0 si aucune n'est rencontrée
 */
function selectedSphere(pos, spherePos, rayon)
{
    //Tir de la souris dans le canvas
...
function handleMouseMove(event)
{
    ...
    //Traitement en fonction des flags de mouvement
    if (dragging)
    {
        rotY += dX;
        rotX += dY;
...
function handleMouseDown(event)
{
    ...
    //Calcule de la distance entre la caméra et la sphere (si pas d'intersetion: retourne 1000.0 )
    ...
    //On test quel sphere interceptée est la plus proche
    var t = 100.0;
    if (tsphere1 < t)
    {
        t = tsphere1;
    ...
    //En fonction de la sphere touchée on change les flags de selection
    if (t == 100.0)
    {
        dragging = true;
    }
...
/*
 Appelle au relachement du click de la souris
